...
} from '../../reduxState/apis/incomeCategoryApi';
...
const Icategory = () => {
  ...
  const handleOk = async () => {
    if (isDelete) {
      await deleteIncomeCategory({ incomeCategoryId: isDelete })
        .unwrap()
        .then((res) => {
          if (res.status) {
    ...
    },
    {
      ...
      render: (_, record: any) => (
        <div className="flex items-center space-x-3.5">
          <EditButton
            handleClick={() => navigate(`/Icategory/edit/${record._id}`)}
          />
  ...
  const data: DataType[] = isSuccess
    ? incomeCategorydata.map((val: Iicategory, index: number) => {
